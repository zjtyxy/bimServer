...
import com.ciat.bim.server.usagerecord.ApiUsageStateService;
...
import org.jeecg.modules.rpc.controller.rpc.HttpValidationCallback;
import org.jeecg.modules.rpc.controller.rpc.ValidationResult;
...
//    protected UserService userService;
    ...
    @Autowired
    protected IDeviceProfileService deviceProfileService;
...
//    protected IAssetService assetService;
    ...
    @Autowired
    protected IRuleChainService ruleChainService;

//    @Autowired
//    protected EntityViewService entityViewService;

    @Autowired(required = false)
    protected IEdgeService edgeService;
...
//    protected RpcService rpcService;
    ...
    public Result<ResponseEntity> validateEntityAndCallback(SecurityUser currentUser, Operation operation, String entityType, String entityIdStr,
                                                                    ThreeConsumer<Result<ResponseEntity>, TenantId, EntityId> onSuccess) throws ThingsboardException {
    ...
    public Result<ResponseEntity> validateEntityAndCallback(SecurityUser currentUser, Operation operation, String entityType, String entityIdStr,
                                                                    ThreeConsumer<Result<ResponseEntity>, TenantId, EntityId> onSuccess,
                                                                    BiConsumer<Result<ResponseEntity>, Throwable> onFailure) throws ThingsboardException {
        return validateEntityAndCallback(currentUser, operation, EntityIdFactory.getByTypeAndId(entityType, entityIdStr),
                onSuccess, onFailure);
    }

    public Result<ResponseEntity> validateEntityAndCallback(SecurityUser currentUser, Operation operation, EntityId entityId,
                                                                    ThreeConsumer<Result<ResponseEntity>, TenantId, EntityId> onSuccess) throws ThingsboardException {
    ...
    public Result<ResponseEntity> validateEntityAndCallback(SecurityUser currentUser, Operation operation, EntityId entityId,
        ...
        validate(currentUser, operation, entityId, new HttpValidationCallback(response,
                new FutureCallback<Result<ResponseEntity>>() {
                    @Override
                    public void onSuccess(@Nullable Result<ResponseEntity> result) {
                        try {
    ...
    private void validateDevice(final SecurityUser currentUser, Operation operation, EntityId entityId, FutureCallback<ValidationResult> callback) {
        if (currentUser.isSystemAdmin()) {
            ...
            ListenableFuture<Device> deviceFuture = deviceService.getByIdFuture(entityId.getId());
...
//        }
    ...
    public static void handleError(Throwable e, final Result<ResponseEntity> response, HttpStatus defaultErrorStatus) {
