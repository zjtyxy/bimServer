...
//                                TopicPartitionInfo tpi = partitionService.resolve(ServiceType.TB_CORE, tenantId, DeviceId.fromString( device.getId()));
...
//            ListenableFuture<List<AttributeKv>> tsData = tsService.findLatest(TenantId.SYS_TENANT_ID, device.getId(), PERSISTENT_ATTRIBUTES);
//            future = Futures.transform(tsData, extractDeviceStateData(device), deviceStateExecutor);
//        } else {
//            ListenableFuture<List<AttributeKv>> attrData = attributesService.find(TenantId.SYS_TENANT_ID, device.getId(), DataConstants.SERVER_SCOPE, PERSISTENT_ATTRIBUTES);
...
//    private <T extends AttributeKv> Function<List<T>, DeviceStateData> extractDeviceStateData(Device device) {
//        return new Function<>() {
//            @Nonnull
//            @Override
...
//    private long getEntryValue(List<? extends AttributeKv> kvEntries, String attributeName, long defaultValue) {
//        if (kvEntries != null) {
//            for (AttributeKv entry : kvEntries) {
...
//    private boolean getEntryValue(List<? extends AttributeKv> kvEntries, String attributeName, boolean defaultValue) {
//        if (kvEntries != null) {
//            for (AttributeKv entry : kvEntries) {
//                if (entry != null && !StringUtils.isEmpty(entry.getKey()) && entry.getKey().equals(attributeName)) {
