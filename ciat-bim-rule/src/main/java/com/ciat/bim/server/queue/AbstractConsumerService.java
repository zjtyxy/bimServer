        ...
        if (actorMsgOpt.isPresent()) {
            ...
            if (actorMsg instanceof ComponentLifecycleMsg) {
                ...
                if (EntityType.TENANT_PROFILE.equals(componentLifecycleMsg.getEntityId().getEntityType())) {
                    ...
                    tenantProfileCache.evict(TenantId.fromString(componentLifecycleMsg.getTenantId()));
                    if (componentLifecycleMsg.getEvent().equals(ComponentLifecycleEvent.UPDATED)) {
                        apiUsageStateService.onTenantUpdate(TenantId.fromString(componentLifecycleMsg.getTenantId()));
                    ...
                    apiUsageStateService.onApiUsageStateUpdate(TenantId.fromString(componentLifecycleMsg.getTenantId()));
