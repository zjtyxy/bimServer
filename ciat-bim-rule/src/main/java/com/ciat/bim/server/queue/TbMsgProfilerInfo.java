...
public class TbMsgProfilerInfo {
    ...
    public TbMsgProfilerInfo(String msgId) {
        this.msgId = msgId;
    }
    ...
    public Map.Entry<String, Long> onTimeout() {
        long currentTime = System.currentTimeMillis();
        stateLock.lock();
        try {
            if (currentRuleNodeId != null && stateChangeTime > 0) {
                ...
                return new AbstractMap.SimpleEntry<String, Long>(currentRuleNodeId.getId(), timeoutTime);
            }
        } finally {
            stateLock.unlock();
        }
