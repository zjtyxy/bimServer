...
public abstract class AbstractSqlTimeseriesDao extends BaseAbstractSqlTimeseriesDao implements AggregationTimeseriesDao {
    ...
    protected ListenableFuture<List<TsKv>> processFindAllAsync(TenantId tenantId, EntityId entityId, List<ReadTsKvQuery> queries) {
        List<ListenableFuture<List<TsKv>>> futures = queries
                .stream()
                .map(query -> findAllAsync(tenantId, entityId, query))
                .collect(Collectors.toList());
        return Futures.transform(Futures.allAsList(futures), new Function<List<List<TsKv>>, List<TsKv>>() {
            @Nullable
            @Override
            public List<TsKv> apply(@Nullable List<List<TsKv>> results) {
    ...
    protected int getDataPointDays(TsKv tsKvEntry, long ttl) {
